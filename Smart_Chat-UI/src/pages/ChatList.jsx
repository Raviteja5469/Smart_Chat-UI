...
export default function ChatList({ chats = [], selectedChatId, onChatSelect, onNewChat }) {
  ...
  const filteredChats = Array.isArray(chats)
    ? chats.filter(chat => {
        ...
        const nameMatch = other?.name?.toLowerCase().includes(search.toLowerCase());
        const lastMsgMatch = chat.lastMessage?.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="w-full lg:w-80 bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl font-bold text-gray-900 ">chatNetic</h1>
          <button
            onClick={onNewChat}
